var calcModule = (function () {
	...
	return {
		 ...
		 * @param {Callback} cbView 	- Binding View Callback
		 */
		addNum (val, cbView) {
			...
			if (!isNaN(val) && len < this.define.DEFINEOPTION.numLimit) {
		...
		delNum (val, cbView) {
			...
			if (curNumber.length > 0) {
				...
				if (curNumber.length === 1 && curNumber[0] === humanKeyMap.Minus) {
					_deleteNumber();
				}
		...
		operator (val, cbView) {
			...
			// 연산된 결과가 있으면 Bind 된 View 를 호출하여 출력한다.
			if (numberVal || formulaVal) {
				cbView(numberVal, formulaVal);
			}
		},
		/**
		 * 계산기 종료 (Clear) 함수
		...
		setEqual () {
			...
			if (equalVal !== this.define.VALERROR) {
		...
		setArithmeticOperation (optVal) {
			...
			if (!numberLen && formulaLen) {
				...
				if (resultVal !== this.define.VALERROR) {
		...
		setAutoOperation (optVal) {
			let {curNumber, lastResult} = calcStorage,
				...
				isAddLastResult = false,
				numberVal, targetNumber;

			if (curNumber.length) {
				// 등록된 숫자가 있으면 자동 연산을 수행하고, Model 정보도 갱신한다.
				targetNumber = this.getCurNumber();
				numberVal = String(targetNumber * optNum);
				_replaceNumber([...numberVal]);
				numberVal = this.getCurNumber();
			} else {
				// 등록된 숫자가 없으면 최종 연산값으로 자동 연산을 수행하고 최종 연산값을 갱신한다.
				targetNumber = lastResult;
				numberVal = targetNumber * optNum;
				_addLastResult(numberVal);
				isAddLastResult = true;
			}

			// Percentage 는 Equal 과 동일한 처리를 한다.
			if (optNum === 0.01) {
				if (!isAddLastResult) {
					_addLastResult(numberVal);
				}
				...
				// history 에 등록하고 연산 Model (curNumber, curFormula) 은 초기화 한다.
				_setHistory(`${targetNumber} ${humanKeyMap.Multiply} ${optNum}`, numberVal);
				this.setAllClear();
			}
		...
		convertLocaleStringNumber (num) {
			...
			if (!isNaN(convertNum) && convertNum !== null) {
				...
				strIntNum = Number(arrNum[0]).toLocaleString(this.define.DEFINEOPTION.locale);

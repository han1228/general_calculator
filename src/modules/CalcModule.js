...
let CalcModule = (function () {
	...
	return {
		...
		addNum (val) {
			...
			if (!isNaN(val) && len < CalcDefine.DEFINEOPTION.numLimit) {
		...
		clear (val) {
			...
			return this.makeResultObj(this.getCurNumber(true));
		},
		...
		setEqual () {
			...
			if (equalVal !== CalcDefine.VALERROR) {
		...
		setArithmeticOperation (optVal) {
			...
			if (!numberLen && formulaLen) {
				...
				if (resultVal !== CalcDefine.VALERROR) {
			...
			if (arrFormula.length > 1) {
				for (strVal of arrFormula) {
					if (isNaN(strVal) && this.isOperator(strVal)) {
						...
						try {
							resultVal = operator 
										//? window.eval(resultVal + operator + strVal)
										? window.Function('"use strict";return (' + resultVal + operator + strVal + ')')()
										: Number(strVal);
						} catch (excep) {
							resultVal = CalcDefine.VALERROR;
							break;
						}
					}
				}
		...
		convertLocaleStringNumber (num) {
			...
			if (!isNaN(convertNum) && convertNum !== null) {
				...
				strIntNum = Number(arrNum[0]).toLocaleString(CalcDefine.DEFINEOPTION.locale);
